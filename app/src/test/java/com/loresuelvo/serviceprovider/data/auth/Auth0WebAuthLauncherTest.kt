...
class Auth0WebAuthLauncherTest {
    ...
    @Test
    fun signup_configures_provider_connection_and_auth0_parameters() {
        val builder = configuredBuilder()

        builder.configureSignup(config(providerDatabaseConnection = SYNTHETIC_CONNECTION))
    ...
    @Test
    fun signup_omits_provider_connection_when_configuration_is_blank() {
        val builder = configuredBuilder()

        builder.configureSignup(config(providerDatabaseConnection = "  "))
    ...
    fun google_login_keeps_its_dedicated_connection() {
        ...
        builder.configureGoogleLogin(config(providerDatabaseConnection = SYNTHETIC_CONNECTION))
        ...
        verify(exactly = 0) { builder.withConnection(SYNTHETIC_CONNECTION) }
    }
    ...
    private fun config(providerDatabaseConnection: String): Auth0Config = Auth0Config(
        domain = "synthetic.auth0.com",
        clientId = "synthetic-client-id",
        scheme = SYNTHETIC_SCHEME,
        audience = SYNTHETIC_AUDIENCE,
        providerDatabaseConnection = providerDatabaseConnection,
    )
