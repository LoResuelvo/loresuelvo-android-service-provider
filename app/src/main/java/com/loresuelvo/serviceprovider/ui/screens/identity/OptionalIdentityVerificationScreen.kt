...
@Composable
private fun IdentityScanIllustration() {
    val scan = remember { Animatable(0f) }
    var completed by remember { mutableStateOf(false) }
    LaunchedEffect(Unit) {
        scan.animateTo(1f, tween(durationMillis = 1600))
        completed = true
    }
    ...
    Box(
        ...
        Box(modifier = Modifier.size(width = 260.dp, height = 172.dp), contentAlignment = Alignment.Center) {
            Surface(
